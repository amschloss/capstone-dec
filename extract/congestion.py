...
with requests.Session() as s:
    ...
    while batch_count == num_records and counter < 10:
        ...
        rq_params = {"$limit": num_records, "$offset": offset, "$order": "time"}
        result = s.get("https://data.cityofchicago.org/resource/sxs8-h27x.json", params = rq_params)
        json_res = result.json()
        with open(f"../data/congestion_{counter}.json", "w") as f:
